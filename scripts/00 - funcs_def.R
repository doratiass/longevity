...
cal_plot_three <- function(final_fit, train_fit, split = c("train", "test"), plat = FALSE) {
  ...
  df %>%
    ...
    geom_smooth(linewidth = line_size, method = "loess", se = TRUE, fullrange = TRUE) +  # Draw smooth calibration curves.
    geom_abline(intercept = 0, slope = 1, linetype = "longdash") +  # Add a reference line for perfect calibration.
    geom_point(data = plot_txt, aes(x = 0.5, y = 0.2 - 0.05 * inx), shape = 15, size = 3) +  # Plot points for annotations.
    geom_text(data = plot_txt, aes(x = 0.53, y = 0.2 - 0.05 * inx, label = txt, size = 15),
              color = "black", hjust = 0) +  # Add text annotations with computed coefficients.
...
cal_scam_plot_three <- function(final_fit, train_fit, split = c("train", "test"), plat = FALSE) {
  ...
  df %>%
    ...
    geom_smooth(linewidth = line_size, method = "loess", se = TRUE, fullrange = TRUE) +  # Calibration curve.
    geom_abline(intercept = 0, slope = 1, linetype = "longdash") +  # Reference line for perfect calibration.
    geom_point(data = plot_txt, aes(x = 0.5, y = 0.2 - 0.05 * inx), shape = 15, size = 3) +  # Annotation points.
    geom_text(data = plot_txt, aes(x = 0.53, y = 0.2 - 0.05 * inx, label = txt, size = 15),
              color = "black", hjust = 0) +  # Annotation text with coefficients.
    labs(x = "Predicted risk",
         y = "Observed proportion") +
    theme_bw() +
    coord_equal(xlim = c(0, 1), ylim = c(0, 1)) +
