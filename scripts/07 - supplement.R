...
ml_df %>%
  mutate(
    across(c("dmg_SES", "dmg_num_room"), as.numeric),
    outcome = ifelse(outcome == "centenarian", "Centenarian", "Not centenarian")
  ) %>%
  select(
    "dmg_admission_age",
    "dmg_origin",
  ...
  tbl_summary(
    by = "outcome",
    type = list(
      `Socioeconomic Status quartile` ~ "continuous",
...
tibble(
  vars = colnames(ml_df),
  values = case_when(
    sapply(ml_df, is.factor) ~
      map_chr(ml_df, ~ paste(unique(.x, na.rm = TRUE), collapse = ", ")),
    sapply(ml_df, is.numeric) ~
      map_chr(
        ml_df,
        ~ paste(range(as.numeric(.x), na.rm = TRUE), collapse = " - ")
      ),
    TRUE ~ NA_character_
  ),
...
step_model$anova %>%
  ...
  mutate(
    Variable = str_split_i(Variable, "\\+ ", i = 2),
...
final_lasso_fit %>%
  ...
  filter(Variable != "(Intercept)", Estimate > 0) %>%
  mutate(
    Variable = map_chr(Variable, label_all),
...
left_join(
  ...
  final_lasso_fit %>%
    collect_predictions() %>%
    select(.row, pred_lasso = .pred_centenarian),
  by = ".row"
) %>%
  left_join(
    final_log_fit %>%
      collect_predictions() %>%
      select(.row, pred_log = .pred_centenarian),
    by = ".row"
  ) -> final_preds
...
# Figure 1 - Pre-Post Calibration Plots ---------------------------------------
# ============================================================================ #
pre_cal_train <- cal_scam_plot_three(
...
post_cal_train <- cal_scam_plot_three(
  list(final_log_fit, final_lasso_fit, final_xgb_fit),
  list(log_train_fit, lasso_train_fit, xgb_train_fit),
  split = "train",
  plat = TRUE
...
pre_cal_test <- cal_scam_plot_three(
  list(final_log_fit, final_lasso_fit, final_xgb_fit),
  list(log_train_fit, lasso_train_fit, xgb_train_fit),
  split = "test",
  plat = FALSE
...
post_cal_test <- cal_scam_plot_three(
  list(final_log_fit, final_lasso_fit, final_xgb_fit),
  list(log_train_fit, lasso_train_fit, xgb_train_fit),
  split = "test",
  plat = TRUE
...
ggsave(
  filename = file.path("graphs", "supp_fig1.png"),
