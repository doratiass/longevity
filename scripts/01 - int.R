...
clean_df <- raw_data %>%
  transmute(
    ...
    ), ordered = TRUE, levels = c("Very Often", "Sometimes", "Seldom", "Never")),
    ...
    dmg_crisis_father_death = factor(case_when(
      q401 == 1 ~ "No crisis",
      q401 > 1 ~ "Crisis"
    ...
    dmg_concentration_camp = factor(case_when(
      q403 == 1 ~ "Never was",
      q403 != 1 ~ "Was"
...
bmi_sbp_data <- clean_df %>%
  ...
  ungroup() %>%
  select(id_nivdaki, med_sbp_mean, med_sbp_sd, med_dbp_mean, med_dbp_sd,
