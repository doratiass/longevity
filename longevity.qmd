...
load("raw_data/ml_df.RData")
load("raw_data/model_data.RData")
load("raw_data/lasso_prep.RData")
```
...
This data frame contains `r ncol(ml_df)` variables:

1.  Demographics data (`r sum(str_detect(colnames(ml_df),"dmg"))`)

2.  Medical data including medical history, laboratory tests and measurements (`r sum(str_detect(colnames(ml_df),"med"))`)

3.  physical (`r sum(str_detect(colnames(ml_df),"physical"))`)

4.  work (`r sum(str_detect(colnames(ml_df),"work"))`)

5.  family (`r sum(str_detect(colnames(ml_df),"family"))`)

6.  diet (`r sum(str_detect(colnames(ml_df),"diet"))`)
